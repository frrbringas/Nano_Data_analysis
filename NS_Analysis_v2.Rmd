...
Fork_nucleosomes <- BC01_Nucleosomes %>% 
  ...
  filter(StrandStatus == c("Lead", "Lag"), StrandDirection == c("revLead", "fwdLag", "fwdLead", "revLag"))
Fork_nucleosomes
```
#Ploting the barchart
```{r}
...
```
```{r}
p4 <- ggplot(Widoms_unique, aes(x = `Region of Interest`, y = percentage, fill = StrandStatus)) +
  geom_col(
    stat = "identity",
     color = "black",
    size = 0.25,
